...
//        U.print("Entering buildW2vModel");
        File file;
        ...
        try {
            U.print("Building W2v Model");
            DataInputStream data_in = new DataInputStream(new BufferedInputStream(new FileInputStream(U.rootPath + "/" + file)));
			String firstLine = data_in.readLine();
			String[] nums = firstLine.split(" ");
			numberOfWordsInVector = Integer.parseInt(nums[0]);
			numberOfDimensionsInVector = Integer.parseInt(nums[1]);
...
//
//        U.print("Performing W2v Analogy: [" + oldSentiment + "] is to [" + newSentiment + "] as [" + oldWord + "] is to ?");
//
...
//
//        U.print("\tThe best analogous word was: [" + closest_word + "]");
//
...
//    }
    ...
    public static Map<Double, String> pointToStrings(W2vPoint referencePoint,
        ...
        if (number_of_suggestions_to_show > 1 && !word.equals("NO_WORD_YET"))
            U.print("Finding word2vec suggestions similar to [" + word + "]...");
        ...
        for (a = 0; a < number_of_suggestions_to_show; a++) {
            if (a == 0 && number_of_suggestions_to_show > 1)
                U.print("word2vec found " + number_of_suggestions_to_show + " filterWords: ");
...
//
//        U.print("Summing W2v points: ");
//        int i = 0;
//        for (W2vPoint point : points) {
//            if (i >= nOfPoints - 1)
//                U.print("[" + point.getClosestString() + "]");
//            else
//                U.print("[" + point.getClosestString() + "], ");
//            i++;
//        }
...
//
//        U.print("\tThe summed filterWords gave the word: [" + closest_word + "]");
//
...
////
////        U.print("Finding point for " + word + "...");
////
...
////        return new W2vPoint(point, word, new long[] {inputWordVocabPosition}, number_of_input_words);
    ...
    private static void postPrint(OperationType type, String closest_word) {
        switch (type) {
            case SUM:
                U.print("\tThe summed filterWords added up to the word: [" + closest_word + "]");
                break;
            case AVERAGE:
                U.print("\tThe averaged filterWords gave the word: [" + closest_word + "]");
                break;
            case ANALOGY:
                U.print("\tThe best analogous word was: [" + closest_word + "]");
                break;
            case SINGLE:
                U.print("\tFound point for: [" + closest_word + "]...");
                break;
        }
    }
